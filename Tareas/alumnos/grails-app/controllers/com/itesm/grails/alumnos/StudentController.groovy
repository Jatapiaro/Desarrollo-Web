...
class StudentController {
    ...
    def index(Integer max) {
        params.max = Math.min(max ?: 10, 100)
            if(params.query){
                def listAlumno =  Student.withCriteria{
                    or{
                        ilike('matricula', '%'+params.query+'%')
                        ilike('nombreCompleto', '%'+params.query+'%')
                        ilike('correoElectronico', '%'+params.query+'%')
                    }
                }
                respond  listAlumno,  model:[studentCount: listAlumno.size()] 
            }else{
    ...
    def save(Student student) {
        ...
        student.matricula=matriculaCreate(student.id)
        student.save flush:true
    ...
    String matriculaCreate(long id){
        String mat=""+id;
        int size=mat.length();
        if(size==7){
            return "A0"+mat;
